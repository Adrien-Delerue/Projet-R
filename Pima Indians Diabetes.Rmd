...
`cleaned_outcome_diabetes` is our data set before the first step, so not cleaned at all. with missing values and incoherences (only outcomes as been associated to strings).
...
The **blue curve** represen non-diabetic's patient. Their curve is centered around lower glucose values. In the other hand, the **red curve** are diabetic's patient is shifted to the right, indicating higher glucose levels. The blue curve extends further right, suggesting greater variability in glucose levels among diabetics.
...
We can visualize this graphe for each column available
