...
```{r include=FALSE}
# install.packages(c("ggplot2", "dplyr", "tidyr", "readr", "lubridate", "caret"))
library(dplyr)
library(gridExtra)
library(ggplot2)
library(reshape2)
...
We can see that when both the Diabetes Pedigree Function and Age are low, there is still a considerable chance of being diabetic. However, the two colors are heavily mixed, so we cannot distinguish a clear separation between the two classes based on these two features alone.

We can visualize this graph for each column available
...
Here we have two main clusters appear. Non-diabetic individuals (blue) mostly occupy the lower-left, with low Insulin and thin skinfolds. Diabetic individuals (pink) are more spread out, showing higher Insulin and more variable Skin Thickness.
...
The plot reveals two main clusters: on the left, a blue cluster representing mostly non-diabetic individuals (with relatively low glucose levels and blood pressure), and on the right, a pink cluster representing individuals with diabetes.
...
| Logistic Prediction | Diabetic | Not Diabetic |
|---------------------|----------|--------------|
| Not Diabetic        | 23       | 90           |
| Diabetic            | 29       | 12           |
...
| K-Nearest Neighbors | Diabetic | Not Diabetic |
|---------------------|----------|--------------|
| Not Diabetic        | 15       | 87           |
| Diabetic            | 35       | 15           |
