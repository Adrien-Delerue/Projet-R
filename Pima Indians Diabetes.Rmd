...
date: "2025-10-22"
author:
- Cauche Paul - Coppin Léo- Delerue Adrien - Mathieu Paul
---
...
```{r include=FALSE}
# install.packages(c("ggplot2", "dplyr", "tidyr", "readr", "lubridate", "caret", "class"))
library(dplyr)
library(gridExtra)
library(ggplot2)
library(reshape2)
...
We have 8 variables and an outcome, Here is how to interpret them:
...
**Insulin** : 2-Hour serum insulin (µ\*U/ml)
...
**Outcome**: variable indicating whether a person is diabetic (`1`) or not (`0`)

# Step 1 : Data Cleaning and Preparation

Looking closely at the summary of our dataset, we can see misprint. The minimum values for glucose level, blood pressure, skin thickness, BMI, insulin, and diabetes pedigree function are 0. This makes no sense, so we need to clean up our dataset.

Now that we have seen theses errors, we are going to replace all the impossible values with a NA value, such as :

-   All Glucose level values at 0 in NA

-   All the Blood Pressure at 0 in NA

-   All the Skin Thickness at 0 in NA

-   All the BMI at 0 in NA

-   All the Insuline at 0 in NA

-   And all the Diabetes Pedigree Function at 0 in NA

We obtain the following summary :

```{r echo=FALSE}
# Mutating each columns concern, if a value equals 0, it's replaced by an NA value
selected_col <- c("Glucose", "BloodPressure", "SkinThickness", "BMI", "DiabetesPedigreeFunction", "Insulin")
diabetes <- raw_diabetes %>% mutate(across(all_of(selected_col), ~ na_if(., 0))) 
# Replace all 0 values with NA in the selected columns
...
Under the columns, you can see the number of NA values that have been placed, which correspond to the number of errors/misinformation that existed before the operation.

Now we are going to replace those NA values by the median of each columns. We chose to replace by the median because it isn't affected by extreme values like the mean so it's closer to reality. We obtain the following summary :
...
`renamed_outcome_diabetes` is our data set before the first step, so not cleaned at all with missing values and incoherences (only outcomes as been associated to strings).

```{r echo=FALSE}
renamed_outcome_diabetes <- raw_diabetes %>% mutate(
  Outcome = case_when(
    Outcome == 1 ~ "Diabetic",
    .default = "Not Diabetic"
  )
)
p1 <- ggplot(renamed_outcome_diabetes, aes(x = Glucose, fill = as.factor(Outcome))) +
...
The **blue curve** represent non-diabetic's patient. Their curve is centered around lower glucose values. In the other hand, the **red curve** are diabetic's patient is shifted to the right, indicating higher glucose levels. The blue curve extends further right, suggesting greater variability in glucose levels among diabetics.
...
grid.arrange(p3, p4, ncol = 2, top = "With cleaned data")
```
...
```{r echo=FALSE}
p5 <- ggplot(renamed_outcome_diabetes, aes(x = as.factor(Outcome), y = BMI, 
                                        fill = as.factor(Outcome))) +
  geom_boxplot() +
  labs(title = "BMI by Outcome", x = "Outcome", y = "BMI") +
  theme_minimal()

p6 <- ggplot(renamed_outcome_diabetes, aes(x = as.factor(Outcome), y = Age, 
                           fill = as.factor(Outcome))) +
  geom_boxplot() +
  labs(title = "Age by Outcome", x = "Outcome", y = "BMI") +
  theme_minimal()

grid.arrange(p5, p6, ncol = 2, top = "With not cleaned data")
```
...
We can see that **age and pregnancy** are fairly correlated, which makes sense. Generally, the older you are, the more likely you are to have been pregnant. In a same way with **BMI & SkinThickness** variable: These measurements are physiologically related

Despite these small correlations (barely exceeding 0.5). We observe the absence of severe multicollinearity indicates that all variables can be safely included in our predictive models without redundancy issues.

```{r echo=FALSE}
ggplot(diabetes, aes(x = Age, y = DiabetesPedigreeFunction, color = factor(Outcome))) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "Age vs Diabetes Pedigree Function",
    x = "Age",
...
We can see that when both the Diabetes Pedigree Function and Age are low, there is still a considerable chance of being diabetic. However, the two colors are heavily mixed, as the correlation between both is 0.03, so we cannot distinguish a clear separation between the two classes based on these two features alone.

We can visualize this graph for each pair of columns available

```{r echo=FALSE}
ggpairs(
  data = diabetes,
  columns = colnames(numerical_vars),
  mapping = aes(color = factor(Outcome), alpha = 0.6)
)
```

Here, we can observe some interesting plots, such as Skin Thickness vs. BMI and Glucose vs. Blood Pressure. So let's zoom on them :

```{r echo=FALSE}
ggplot(diabetes, aes(x = Insulin, y = SkinThickness, color = factor(Outcome))) +
  geom_point(alpha = 0.5, size = 5) +
  labs(
    title = "Insulin vs SkinThickness",
    x = "Insulin",
    y = "SkinThickness"
  ) +
  theme_minimal()
...
Here we have two main clusters appear. Non-diabetic individuals (blue) mostly occupy the lower-left, with low Insulin and thin skinfolds. Diabetic individuals (red) are more spread out, showing higher Insulin and more variable Skin Thickness.

```{r echo=FALSE}
ggplot(diabetes, aes(x = Glucose, y = BloodPressure, color = factor(Outcome))) +
  geom_point(alpha = 0.5, size = 5) +
  labs(
...
The plot reveals two main clusters: on the left, a blue cluster representing mostly non-diabetic individuals (with relatively low glucose levels and blood pressure), and on the right, a red cluster representing individuals with diabetes.
...
We are going to use 2 different algorithms, logistic Regression Model and K-nearest Model. Then we'll use Cross-Validation on them, and finally we'll analyze the impact of cleaning.
...
| Logistic Regression    | Real Diabetic | Real Not Diabetic |
|------------------------|---------------|-------------------|
| Predicted Not Diabetic | 23            | 90                |
| Predicted Diabetic     | 29            | 12                |

The lines on the table are the reality and the columns are the predicted. Our model predicted 119 (90+29) right out of 154 (77% of accuracy) and it predicted 12 false positive and 23 false negative.
...
| K-Nearest Neighbors     | Real Diabetic | Real Not Diabetic |
|-------------------------|---------------|-------------------|
| Predicated Not Diabetic | 17            | 87                |
| Predicated Diabetic     | 35            | 15                |

The lines on the table are the reality and the columns are the predicted. Our model predicted 122 (87+35) right out of 154 (79% of accuracy), it also predicted 17 false positive and 15 false negative.

### Cross Validation

We trained and tested our model with a 80% of the data and 20% to test it. Now we are going to observe the results for each of our models with a different split to evaluate their robustness and define which one is better.

For that we are going to divide our dataset in 15 equal parts and train our models 15 times. On each training we'll train the model on 14 folds then test it on the 15th.
...
for(i in 1:n) {
  test_fold  <- folds[[i]]
  train_fold <- do.call(rbind, folds[-i]) # The others
  
  # Logistic Regression Model 
  logistic_model <- glm(Outcome ~ ., data = train_fold, family = binomial)
  # Prediction on our test data (binarization)
  ...
  # Registering the number of false positive and false negative to analyse the impact of data cleanin
  falsePositiveLR <- c(falsePositiveLR, sum(test_fold$Outcome == 0 & logistic_prediction == 1))
  
  falseNegativeLR <- c(falseNegativeLR, sum(test_fold$Outcome == 1 & logistic_prediction == 0))
...
```{r include=FALSE}
# Preparing the data to be treated by the algorithms
trainDirty <- raw_diabetes %>% mutate(across(where(is.numeric) & !last_col(), scale))
...
accuracyLRDirty <- c()
falsePositiveLRDirty <- c()
falseNegativeLRDirty <- c()
...
for(i in 1:n){
  ...
  falsePositiveLRDirty <- c(falsePositiveLRDirty, sum(test_fold$Outcome == 0 & logistic_prediction == 1))
  
  falseNegativeLRDirty <- c(falseNegativeLRDirty, sum(test_fold$Outcome == 1 & logistic_prediction == 0))
}
...
We can observe that there the Model's accuracy is more stable when using the cleaned data and therefore more robust.

To have another way of comparing the use of cleaned and dirty data we are also going to compare the number of false Positive and false Negative of our model.

```{r echo=FALSE}
df <- data.frame(
  Fold = 1:n,
  dirty_data = falseNegativeLRDirty,
  cleaned_data = falseNegativeLR
)

ggplot(df, aes(x=Fold)) +
  geom_line(aes(y=dirty_data, color="Dirty Data")) +
  geom_line(aes(y=cleaned_data, color="Cleaned Data")) +
  ylim(0,nrow(raw_diabetes) / 15) +
  labs(title="Dirty Data vs Cleaned Data False Negative", y="False Negative") +
  scale_color_manual("", values=c("Dirty Data"="blue", "Cleaned Data"="red"))
```
...
df <- data.frame(
  Fold = 1:n,
  dirty_data = falsePositiveLRDirty,
  cleaned_data = falsePositiveLR
)
...
ggplot(df, aes(x=Fold)) +
  ...
  geom_line(aes(y=cleaned_data, color="Cleaned Data")) +
  ylim(0,nrow(raw_diabetes) / 15) +
  labs(title="Dirty Data vs Cleaned Data False Positive", y="False Positive") +
  scale_color_manual("", values=c("Dirty Data"="blue", "Cleaned Data"="red"))
```

We can, for the false Positive and the false Negative, that there is less variation when using the cleaned data. Therefore, we can deduce that the model using cleaned data is more robust and stable than the one using the dirty data.

This is the same conclusion we had previously.

In the end the use of Cleaned data is better as it avoid over-fitting and provide a better generalization of our model.
