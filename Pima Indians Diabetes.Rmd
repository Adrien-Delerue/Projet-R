...
```{r echo=FALSE}
ggplot(diabetes_scaled, aes(x = Age, y = DiabetesPedigreeFunction, color = factor(Outcome))) +
  geom_point(alpha = 0.7, size = 2) +
  scale_color_manual(values = c("blue", "red"), 
                     name = "Outcome",
                     labels = c("No Diabetes", "Diabetes")) +
...
```{r echo=FALSE}
pairs_columns = colnames(raw_diabetes)
pairs_columns = pairs_columns[pairs_columns != "Outcome"]
ggpairs(
  data = diabetes,
